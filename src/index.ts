...
import { Rover } from "./utils/types.js";
import {
  createBounds,
  buildRovers,
  directionsArr,
...
//Main function
export default function operationStart(input: string[]) {
  const arrayOnlyHasStrings = input.every((item) => typeof item === "string");
  if (!arrayOnlyHasStrings) return "Input must be a string.";
  ...
  for (let i = 0; i < directions.length; i++) {
    movedRoversArr.push(
      executeDirections(directions[i], rovers[i], movedRoversArr)
    );
  }

  for (let n = 0; n < movedRoversArr.length; n++) {
    const inBounds = checkBounds(movedRoversArr[n], plateauBounds);
    if (!inBounds) throw "rover is out of bounds.";
  }
...
rl.on("line", function (cmd) {
  if (cmd.toUpperCase() === "GO") rl.close();
  input.push(cmd);
});
